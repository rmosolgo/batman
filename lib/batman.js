...
(function() {
  ...
  Batman.App = (function(_super) {
    ...
    App.run = function() {
      ...
      if (this.get('dataStore') == null) {
        this.set('dataStore', new Batman.DataStore);
      }
...
(function() {
  ...
  Batman.RestStorage2 = (function(_super) {
    __extends(RestStorage2, _super);

    function RestStorage2() {
      _ref = RestStorage2.__super__.constructor.apply(this, arguments);
      return _ref;
    }

    RestStorage2.prototype.fetch = function(options, callback) {
      var _this = this;
      return new Batman.Request({
        url: "/" + this.model.storageKey + "/" + options.id + ".json",
        success: function(json) {
          var record;
          record = Batman.currentApp.dataStore.populate(_this.model, json);
          return typeof callback === "function" ? callback(record) : void 0;
        }
      });
    };

    RestStorage2.prototype.fetchAll = function(options, callback) {
      var _this = this;
      return new Batman.Request({
        url: "/" + this.model.storageKey + ".json",
        success: function(json) {
          var records;
          records = Batman.currentApp.dataStore.populate(_this.model, json);
...
(function() {
  ...
  console.log = function() {
    ...
    for (_i = 0, _len = options.length; _i < _len; _i++) {
      option = options[_i];
      originalLog.call(console, (option != null ? option.valueForBinding : void 0) ? option.valueForBinding() : option);
    }
    return null;
  };

  Batman.Query = (function(_super) {
    ...
    Query.prototype.where = function(options) {
      return this.options.where = options;
    };

    Query.prototype.distinct = function(value) {
      return this.options.distinct = typeof value === 'undefined' ? true : !!value;
    };

    Query.prototype.uniq = function(value) {
      return this.distinct(value);
    };

    Query.prototype.order = function(key) {
      return this.options.order = key;
    };

    Query.prototype.group = function(key) {
      return this.options.group = key;
    };

    Query.prototype.limit = function(amount) {
      return this.options.limit = amount;
    };

    Query.prototype.offset = function(amount) {
      return this.options.offset = amount;
    };

    Query.prototype._isSingleRecord = function() {
      return (this.options.id != null) || this.options.limit === 1;
    };

    Query.prototype.find = function() {
      var record, records, _i, _len, _results;
      if (this._isSingleRecord()) {
        record = Batman.currentApp.dataStore.querySingle(this.options);
        if (record) {
          return this.options.model.fromRecord(record);
        }
      } else {
        records = Batman.currentApp.dataStore.query(this.options);
        _results = [];
        for (_i = 0, _len = records.length; _i < _len; _i++) {
          record = records[_i];
          if (record) {
            _results.push(this.options.model.fromRecord(record));
          } else {
    ...
    Query.prototype.fetch = function(callback) {
      var adapter;
      this.isFetched = true;
      adapter = this.options.model.storageAdapter();
      if (this._isSingleRecord()) {
        return adapter.fetch(this.options, callback);
      } else {
        return adapter.fetchAll(this.options, callback);
      }
    };

    Query.prototype.valueForBinding = function() {
    ...
    Query.prototype.toString = function() {
      return this.valueForBinding() || "Loading...";
    };
  ...
  Batman.DataStore = (function() {
    function DataStore() {
      this.storage = new Batman.Set;
      this.byModel = {};
    }

    DataStore.prototype.query = function(options) {
      var model;
      if (model = options.model) {
        return this.modelStorage(model).forEach(function(id, record) {
          return record;
        });
      }
    };

    DataStore.prototype.querySingle = function(options) {
      if (options.id != null) {
        return this.modelStorage(options.model).get(options.id);
      }
      if (options.offset < 0) {
        return this.storage[this.storage.length + options.offset];
      }
    };

    DataStore.prototype.modelStorage = function(modelClassOrName) {
      var _base, _name;
      return (_base = this.byModel)[_name = typeof modelClassOrName === 'function' ? modelClassOrName.resourceName : modelClassOrName] || (_base[_name] = new Batman.Hash);
    };

    DataStore.prototype.record = function(model, id, _addedRecords) {
      var modelStorage;
      modelStorage = this.modelStorage(model);
      return modelStorage.getOrSet(id, function() {
        var record;
        record = new Batman.DataStoreRecord(id, {
          model: model
        });
        Batman.currentApp.dataStore.storage.add(record);
        if (_addedRecords != null) {
    ...
    DataStore.prototype.populate = function(model, json) {
      var addedRecords, buildRecord, changedRecords, modelStorage,
        _this = this;
      modelStorage = this.modelStorage(model);
      addedRecords = [];
      changedRecords = [];
      buildRecord = function(data) {
        var record;
        record = _this.record(model, data.id, addedRecords);
        record.mixinClean(data);
        return changedRecords.push(record);
      };
      modelStorage._preventMutationEvents(function() {
        var data, _i, _len;
        if (Batman.typeOf(json) === 'Array') {
      ...
      if (addedRecords.length) {
        modelStorage.fire('itemsWereAdded', addedRecords.map(function(record) {
          return record.id;
        }), addedRecords);
      }
  ...
  Batman.DataStoreRecord = (function() {
    function DataStoreRecord(id, values) {
      this.id = id;
      this._dirtyKeys = [];
      if (values) {
        this.mixinClean(values);
      }
    }

    DataStoreRecord.prototype.mixinClean = function(values) {
      var key, value;
      this.noDirtyTracking = true;
      for (key in values) {
        value = values[key];
        this.set(key, value);
      }
      return this.noDirtyTracking = false;
    };

    DataStoreRecord.prototype.set = function(key, value) {
      if (!this.noDirtyTracking) {
        this._dirtyKeys.push(key);
      }
      return this[key] = value;
    };

    DataStoreRecord.prototype.dirtyKeys = function() {
      return this._dirtyKeys.unique();
    };
...
(function() {
  ...
  Batman.Model = (function(_super) {
    ...
    Model.classAccessor('all', function() {
      return new Batman.Query(this);
    });
    ...
    Model.createFromJSON = function(json) {
      var id, record;
      if (id = json[this.primaryKey]) {
        record = new Batman.Query(this, {
          id: id
        });
      }
    ...
    Model.fromRecord = function(record) {
  ...
  Batman.DOM.AbstractBinding = (function(_super) {
    ...
    AbstractBinding.accessor('filteredValue', {
      get: function() {
        ...
        if (unfilteredValue != null ? unfilteredValue.valueForBinding : void 0) {
